...
def generate_team_df(
    league_abbr, team_abbr, target_fields, target_seasons, table_ids, data_attr
):
...
def generate_league_df(
    league_abbr, target_teams, target_fields, target_seasons, table_ids, data_attr
):
...
def prepare_league_data(df, league, name_mapping, final_ordering, team_name_col_map):
    ...
    # Scraped dates look like Tues, Nov 2, 1999
        ...
        df["game_date"] = df["game_date"].apply(
            lambda d: datetime.datetime.strptime(d, "%b %d, %Y")
        )
    ...
    df["game_id"] = (
        df["game_year"].astype(str)
        + df["game_month"].astype(str)
        + df["game_day"].astype(str)
        + df["team"].astype(str)
        + df["opponent"].astype(str)
        + df["team_pts"].astype(str)
        + df["oppt_pts"].astype(str)
    )

    df["game_id"] = df["game_id"].apply(
        lambda x: "".join(sorted(x.replace(" ", "").upper()))
    )
